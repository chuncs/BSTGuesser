...
public class BinarySearchTree {
    ...
    private TreeNode search(int value) {
        ...
        //current = searchRecursively(current, value);
        while (true) {
    ...
    private TreeNode searchRecursively(TreeNode current, int value) {
        if (current != null) {
            if (current.getValue() == value) {
                return current;
            } else if (value < current.getValue()) {
                current = searchRecursively(current.left, value);
            } else {
                current = searchRecursively(current.right, value);
            }
        }
        return current;
    }
